...
Для локальной разработки можно наполнить БД тестовыми данными с помощью автоматического скрипта установки.

### Требования

**Единственное требование:**
- **Windows:** Установленный [Chocolatey](https://chocolatey.org/install) (для установки зависимостей)
- **Linux/Mac:** Менеджер пакетов (apt-get, yum, brew) с правами sudo
...
1. Проверит и установит все зависимости (ImageMagick, Python Pillow)
2. Проверит, что Docker контейнер запущен
3. Запустит наполнение БД тестовыми данными

**Windows (PowerShell) - Рекомендуется:**
```powershell
# Откройте PowerShell и перейдите в корень проекта
cd C:\Users\user\Desktop\resto-hub

# Запустите скрипт установки (он сам установит все и запустит seed)
.\local-deployment\setup-seed.ps1
```

**Windows (Git Bash) или Linux/Mac:**
```bash
# Откройте Git Bash и перейдите в корень проекта
cd /c/Users/user/Desktop/resto-hub

# Запустите скрипт установки
./local-deployment/setup-seed.sh
```

**WSL:**
```bash
# В WSL перейдите в директорию проекта
cd /mnt/c/Users/user/Desktop/resto-hub
...
**Примечание:** 
- Скрипты можно запускать из любой директории - они автоматически найдут нужные файлы
- На Windows рекомендуется использовать PowerShell скрипт (`setup-seed.ps1`)
- Если при запуске PowerShell скрипта возникает ошибка политики выполнения, выполните: `Set-ExecutionPolicy -ExecutionPolicy RemoteSigned -Scope CurrentUser`

**Что установится автоматически:**
- ImageMagick (через Chocolatey на Windows, через менеджер пакетов на Linux/Mac)
- Python Pillow (через pip)

**Важно:**
- Убедитесь, что Docker контейнер `resto-hub-postgres` запущен: `docker compose up -d`
- На Windows скрипт использует Chocolatey для установки ImageMagick
- Если что-то пойдет не так, скрипт выведет понятные сообщения об ошибках

**Что будет создано в БД:**
- 5 ресторанов с разными кухнями (итальянская, японская, грузинская, европейская, азиатская)
- Меню для каждого ресторана (15-25 блюд)
- Этажи, залы и столы
- Акции и промо-события
- Активные подписки

**Примечание:** Скрипт использует отрицательные ID (-1, -2, -3...) для избежания конфликтов с реальными данными. Скрипт идемпотентен - его можно запускать несколько раз.

### Ручная установка (альтернатива)

Если вы предпочитаете установить зависимости вручную, используйте скрипт `seed-database.sh` напрямую:

```bash
./local-deployment/seed-database.sh
```

В этом случае вам нужно будет самостоятельно установить:
- ImageMagick или Python с Pillow
