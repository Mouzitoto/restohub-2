...
export function RestaurantPage() {
  ...
  const loadRestaurant = async () => {
    ...
    try {
      ...
      // Load promotions
      const apiPromotions = await restaurantApi.getPromotions(restaurantId, { isCurrent: true });
      const mappedPromotions = apiPromotions.map(mapPromotion);
      
      // Load rooms to check isOutdoor and isSmoking
      let isOutdoor = false;
      let isSmoking = false;
      try {
        const rooms = await restaurantApi.getRooms(restaurantId);
        isOutdoor = rooms.some(r => r.isOutdoor);
        isSmoking = rooms.some(r => r.isSmoking);
      } catch (err) {
        // Rooms might not be available, use defaults
      }
      
      const mappedRestaurant = mapRestaurant(apiRestaurant, mappedPromotions.length > 0);
      setRestaurant({
        ...mappedRestaurant,
        isOutdoor,
        isSmoking,
      ...
      if (err.message?.includes('404') || err.message?.includes('RESTAURANT_NOT_FOUND')) {
        setError('Ресторан не найден или неактивен');
      } else {
        ...
        <div className="absolute bottom-0 left-0 right-0 p-4 bg-gradient-to-t from-black/80 to-transparent">
          <div className="flex items-end gap-4">
            {restaurant.logoUrl ? (
              ...
              <div className="w-20 h-20 rounded-xl border-2 border-white bg-white flex items-center justify-center">
