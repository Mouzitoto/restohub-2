...
**Доступ:** ADMIN и MANAGER (`@PreAuthorize("hasAnyRole('ADMIN', 'MANAGER')")`)
...
```json
{
  ...
  "userId": "long (optional, только для ADMIN - ID менеджера, к которому привязать ресторан)"
}
```
...
5. Привязка ресторана к пользователю:
   - **Если роль ADMIN:**
     - Если `userId` указан - привязать ресторан к указанному пользователю через таблицу `users_2_restaurants`
     - Если `userId` не указан - не привязывать ресторан (как было раньше)
   - **Если роль MANAGER:**
     - Игнорировать `userId` (если указан)
     - Автоматически привязать ресторан к текущему менеджеру через таблицу `users_2_restaurants`
     - Получить текущего пользователя из `SecurityContextHolder` по email
6. Возврат созданного ресторана
