...
```json
{
  ...
  "endDate": "date (required, format: YYYY-MM-DD, должна быть >= startDate)",
...
```json
{
  ...
  "endDate": "date (optional, format: YYYY-MM-DD, должна быть >= startDate)",
...
5. Проверка дат: если указаны обе даты, то `endDate >= startDate`
...
9. Возврат обновленного промо-события
...
- Тип: DATE
- Формат: YYYY-MM-DD
- Валидация: `endDate >= startDate` (кастомная проверка)
- Валидатор: `@NotNull`, `@Future` (дата должна быть в будущем)
...
```
- Проверка на уровне класса DTO: `endDate >= startDate`
