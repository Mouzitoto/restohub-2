...
```json
{
  "floorNumber": "integer (required, min: -10, max: 100)"
}
```
...
3. Применение сортировки (по умолчанию по `floor_number ASC`)
...
```json
{
  "floorNumber": "integer (optional, min: -10, max: 100)"
}
```
...
- Тип: INTEGER
- Диапазон: от -10 до 100 (`@Min(-10)`, `@Max(100)`) - для поддержки подвалов и высоких зданий
- Валидатор: `@NotNull`, `@Min(-10)`, `@Max(100)`
