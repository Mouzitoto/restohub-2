...
```json
{
  ...
  "isOutdoor": "boolean (optional, default: false)",
...
3. Для каждого помещения подсчитать количество активных столов:
   - `SELECT COUNT(*) FROM tables WHERE room_id = :roomId AND is_active = true`
   - Добавить поле `tableCount` в ответ
4. Применение фильтра по этажу (если указано `floorId`)
...
- Тип: BOOLEAN
- Валидатор: стандартная валидация boolean
- Описание: указывает, что зал находится на открытом воздухе (веранда, крыша, на улице)
