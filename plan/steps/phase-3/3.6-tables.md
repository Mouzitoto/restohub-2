...
```json
{
  ...
  "tableNumber": "1",
  "capacity": 4,
  "description": "Стол у окна с видом на парк",
  "imageId": 123,
...
3. Применение фильтров:
   ...
   - По другим фильтрам (если указано)
...
```json
{
  ...
  "description": "string | null (optional, TEXT, max 10000)",
...
1. Проверка доступа через интерцептор `/r/*` (для MANAGER)
2. Поиск стола с проверкой принадлежности к ресторану
3. Проверка наличия активных бронирований:
   - Активные бронирования - это бронирования в статусах, означающих подтвержденное или обрабатываемое бронирование (например, 'APPROVED', 'PENDING'), у которых дата бронирования еще не прошла
   - SQL запрос:
     ```sql
     SELECT COUNT(*) FROM bookings b
     JOIN booking_statuses bs ON b.booking_status_id = bs.id
     WHERE b.table_id = :tableId
       AND bs.code IN ('APPROVED', 'PENDING')
       AND (b.date > CURRENT_DATE OR (b.date = CURRENT_DATE AND b.time >= CURRENT_TIME))
     ```
   - Если COUNT(*) > 0 → вернуть ошибку 400 Bad Request с сообщением "Нельзя удалить стол: у стола есть активные бронирования"
   - Формат ошибки: стандартный формат ошибки с `exceptionName` и `message`
4. Мягкое удаление (только если нет активных бронирований):
...
**Ошибки:**
- 404 Not Found: стол не найден, неактивен или не принадлежит ресторану
- 400 Bad Request: стол используется (есть активные бронирования) - с сообщением "Нельзя удалить стол: у стола есть активные бронирования"
- 403 Forbidden: MANAGER не имеет доступа к этому ресторану
