...
- Тело запроса:
  ...
  - `date` - string (required, format: YYYY-MM-DD) - дата бронирования
  - `time` - string (required, format: HH:mm:ss) - время бронирования
  - `personCount` - integer (required, min: 1) - количество персон
  - `clientFirstName` - string (optional, max 255) - "свое имя" клиента
  - `clientName` - string (optional, max 255) - "на чье имя бронировать"
  - `specialRequests` - string (optional, TEXT, max 10000) - особые пожелания
  - `preOrderItems` - array (optional) - массив позиций предзаказа:
    - `menuItemId` - long (required) - ID блюда
    - `quantity` - integer (required, min: 1) - количество
    - `specialRequests` - string (optional, TEXT, max 10000) - особые пожелания к блюду
...
```json
{
  ...
  "whatsappUrl": "https://wa.me/79991234567?text=BOOKING%3A1",
  "message": "BOOKING:1",
...
- Каждая позиция должна содержать:
  - `menuItemId` - обязательное поле, тип LONG
  - `quantity` - обязательное поле, тип INTEGER, минимальное значение: 1
  - `specialRequests` - опциональное поле, тип STRING (TEXT), максимальная длина: 10000
...
**Примечание**: Имена полей в запросе используют camelCase (tableId, clientFirstName, clientName, specialRequests, menuItemId, preOrderItems), что соответствует стандартам JSON API и использованию в client-web.
