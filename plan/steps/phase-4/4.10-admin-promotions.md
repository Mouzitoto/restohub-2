...
- Для каждого события:
  ...
  - Заголовок
  - Тип (badge)
  - Период (дата начала - дата окончания или "Бесконечно" для повторяющихся событий без endDate)
  - Индикатор повторяющегося события (иконка/бейдж "Повторяется", если `isRecurring=true`)
  - Информация о повторении (если `isRecurring=true`): "Каждый четверг", "Ежедневно", "Ежемесячно" и т.д.
...
interface PromotionForm {
  ...
  endDate: string | null;     // optional, date, может быть null для бесконечных повторяющихся событий
  imageId: number | null;     // optional
  isRecurring: boolean;      // optional, default: false
  recurrenceType: 'WEEKLY' | 'MONTHLY' | 'DAILY' | null;  // optional, обязателен если isRecurring=true
  recurrenceDayOfWeek: number | null;  // optional, 1-7 (1=понедельник, 7=воскресенье), обязателен если recurrenceType=WEEKLY
}
```
...
- `endDate`: опциональное, если указано, то >= startDate; может быть null для бесконечных повторяющихся событий
- `description`: max 10000
- `isRecurring`: опциональное, boolean, default: false
- `recurrenceType`: опциональное, обязателен если `isRecurring=true`; допустимые значения: 'WEEKLY', 'MONTHLY', 'DAILY'; должен быть null если `isRecurring=false`
- `recurrenceDayOfWeek`: опциональное, обязателен если `recurrenceType='WEEKLY'`; диапазон 1-7 (1=понедельник, 7=воскресенье); должен быть null если `recurrenceType!='WEEKLY'` или `isRecurring=false`
...
- Условная валидация полей повторения:
  - При включении `isRecurring` показывать селектор `recurrenceType`
  - При выборе `recurrenceType='WEEKLY'` показывать селектор дня недели
  - При `isRecurring=false` скрывать поля повторения и очищать их значения
  - При `isRecurring=true` и отсутствии `endDate` показывать подсказку "Бесконечное повторение"
