...
- `start_date` - DATE NOT NULL (дата начала; для повторяющихся событий - дата начала периода повторений)
- `end_date` - DATE NULL (дата окончания; для повторяющихся событий - дата окончания периода повторений, NULL для бесконечных повторений)
- `image_id` - BIGINT REFERENCES images(id) (изображение промо-события, опционально)
- `is_recurring` - BOOLEAN NOT NULL DEFAULT FALSE (флаг повторяющегося события)
- `recurrence_type` - VARCHAR(50) NULL (тип повторения: WEEKLY, MONTHLY, DAILY; NULL если не повторяющееся)
- `recurrence_day_of_week` - INTEGER NULL (день недели для WEEKLY: 1=понедельник, 2=вторник, ..., 7=воскресенье; NULL если не применимо)
- `is_active` - BOOLEAN NOT NULL DEFAULT TRUE
- `created_at` - TIMESTAMP NOT NULL DEFAULT NOW()
- `updated_at` - TIMESTAMP NOT NULL DEFAULT NOW()
- `deleted_at` - TIMESTAMP NULL
