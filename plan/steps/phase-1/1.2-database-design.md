...
- **booking_statuses** - статусы бронирований и предзаказов (PENDING, APPROVED, REJECTED, CANCELLED)
...
- `id` - BIGSERIAL PRIMARY KEY
- `restaurant_id` - BIGINT NOT NULL REFERENCES restaurants(id) ON DELETE CASCADE
- `floor_number` - INTEGER NOT NULL (номер этажа)
- `is_active` - BOOLEAN NOT NULL DEFAULT TRUE
- `created_at` - TIMESTAMP NOT NULL DEFAULT NOW()
- `updated_at` - TIMESTAMP NOT NULL DEFAULT NOW()
- `deleted_at` - TIMESTAMP NULL
...
- `has_window` - BOOLEAN NOT NULL DEFAULT FALSE (окно)
- `has_air_conditioning` - BOOLEAN NOT NULL DEFAULT FALSE (кондиционер)
- `is_smoking` - BOOLEAN NOT NULL DEFAULT FALSE (курение разрешено)
...
- `id` - BIGSERIAL PRIMARY KEY
- `code` - VARCHAR(50) NOT NULL UNIQUE (код статуса: PENDING, APPROVED, REJECTED, CANCELLED)
